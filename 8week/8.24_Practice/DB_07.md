...
```
Director 테이블에서 country 컬럼의 값이 USA인 데이터가 존재하지 않기 때문에 오류메세지가 나타납니다.
```
...
```
메소드 get은 하나의 데이터만 return 값으로 가져야 한다 Director 테이블에서 country가 KOR인 데이터가 7이므로 return 값이 잘못 나온 오류입니다.
```
...
```
get은 테이블안에서 해당 값이 primary_key 같이 무조건 1개인 경우에 탐색조건으로 해야 합니다. 
filter의 경우 테이블안에서 찾아야 하는 조건이 여러 데이터와 겹치는 경우 같은 조건의 데이터를 탐색하는데 용이합니다.
```
